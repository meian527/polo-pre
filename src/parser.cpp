...
ASTNodePtr Parser::parsePrimary() {
    ...
    switch (currentToken.type) {
        ...
        case TokenType::MINUS: {
            advance();
            return std::make_shared<UnaryOpNode>(line, col, UnaryOpType::Minus, parseExpression());
        }
        case TokenType::REF: {
            advance();
...
std::shared_ptr<EnumDeclNode> Parser::parseEnumDecl() {
    auto line = currentToken.line, col = currentToken.column;
    ...
    if (currentToken.type != TokenType::IDENTIFIER) {
        THROW_ERROR("Expected enum name", currentToken.line, currentToken.column);
    }
    ...
    if (currentToken.type != TokenType::RBRACE) {
        do {
            variants.push_back(parseVariantDecl());
    ...
    auto enum_decl = std::make_shared<EnumDeclNode>(line, col, name, variants);
    enum_decl->is_public = is_public;
    return enum_decl;
}

std::shared_ptr<VariantDeclNode> Parser::parseVariantDecl() {
    auto line = currentToken.line, col = currentToken.column;

    if (currentToken.type != TokenType::IDENTIFIER) {
        THROW_ERROR("Expected variant name", currentToken.line, currentToken.column);
    }
    ...
    return std::make_shared<VariantDeclNode>(line, col, name, types);
}

std::shared_ptr<TraitDeclNode> Parser::parseTraitDecl() {
    auto line = currentToken.line, col = currentToken.column;
    ...
    if (currentToken.type != TokenType::IDENTIFIER) {
        THROW_ERROR("Expected trait name", currentToken.line, currentToken.column);
    }
    ...
    auto trait_decl = std::make_shared<TraitDeclNode>(line, col, name, methods);
    trait_decl->is_public = is_public;
    return trait_decl;
}
...
std::shared_ptr<ImplDeclNode> Parser::parseImplDecl() {
    ...
    return std::make_shared<ImplDeclNode>(line, col, target_type, trait_name, methods);
}
